...
Generates logarithmic-grid prime-counting signals up to 1e9, including theta, psi, and normalized residual channels for spectral analysis.
"""

# ============================================================
# TEIL A: DATENGENERIERUNG BIS 1e9 (segmentiert)
...
# EINSTELLUNGEN
# ------------------------------------------------------------
X_MAX = 1_000_000_000      # 1e9
X_MIN = 10
N_LOG_GRID = 20000         # 16384 oder 20000 ist gut
SEGMENT_SIZE = 5_000_000   # Blockgröße; 2e6 bis 1e7 meist gut
...
def evaluate_theta_psi_on_grid(
    ...
    for seg_low in range(2, x_max + 1, segment_size):
        ...
        if len(primes_seg) == 0:
            # Alle Gridpunkte <= seg_high auf aktuellen running_theta setzen
        ...
        # Alle Gridpunkte innerhalb dieses Segments auswerten
        while grid_ptr < len(x_grid) and x_grid[grid_ptr] <= seg_high:
            xq = x_grid[grid_ptr]
    ...
    # Falls am Ende noch Punkte offen sind
