...
Builds the 1e11 logarithmic-grid prime-side dataset used in the high-scale resonance workflow.
"""

# ============================================================
# TEIL A: DATENGENERIERUNG BIS 1e11 (segmentiert, lean version)
...
#
# Diese Version ist bewusst konservativer für lange Läufe:
#   * kleineres log-grid
#   * größere Segmente
#   * weiterhin vollständige theta/psi-Ausgabe
# ============================================================
...
# EINSTELLUNGEN
# ------------------------------------------------------------
X_MAX = 100_000_000_000      # 1e11
X_MIN = 10
N_LOG_GRID = 12000           # konservativ für 1e11
SEGMENT_SIZE = 20_000_000    # evtl. 50_000_000 testen, falls RAM reicht
...
def evaluate_theta_psi_on_grid(
    ...
    segment_counter = 0
    total_segments = (x_max + segment_size - 1) // segment_size

    for seg_low in range(2, x_max + 1, segment_size):
        seg_high = min(seg_low + segment_size - 1, x_max)
        segment_counter += 1
        if segment_counter % 20 == 0 or segment_counter == 1 or segment_counter == total_segments:
            print(f"Segment {segment_counter}/{total_segments}: [{seg_low:,}, {seg_high:,}]")
