...
Analyzes the 1e10 dataset with spectral summaries, peak extraction, and null-model comparisons.
"""

# ============================================================
# TEIL B: RESONANZANALYSE FÜR 1e10
...
# DATEI LADEN
# ------------------------------------------------------------
DATA_FILE = "lhs_phase_1e10_loggrid.npz"
...
psi = data["psi"].astype(np.float64)
prime_power_corr = data["prime_power_corr"].astype(np.float64)

theta_minus_x = data["theta_minus_x"].astype(np.float64)
...
plt.title("Normalized spectral amplitude vs known zeta ordinates (1e10)")
plt.xlabel("omega")
...
# 7) CSV-EXPORT
# ------------------------------------------------------------
summary_df.to_csv("analysis_global_summary_1e10.csv", index=False)
window_df.to_csv("analysis_window_stability_1e10.csv", index=False)

for name, peaks in peak_tables.items():
    peaks.to_csv(f"analysis_peaks_{name}_1e10.csv", index=False)

print("\nGespeichert:")
print("- analysis_global_summary_1e10.csv")
print("- analysis_window_stability_1e10.csv")
for name in peak_tables:
    print(f"- analysis_peaks_{name}_1e10.csv")
